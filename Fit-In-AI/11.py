...
class WPTEnv:
    def __init__(self, parent, scenario_no, params):
        ...
        self.max_charging = params.task_specific_params.max_charging

        self.battery_no = params.task_specific_params.battery_no
        self.power_no = params.task_specific_params.power_no
        self.charge_no = params.task_specific_params.charge_no
        self.active_no = 2
        ...
        self.charge_partition = np.linspace(0, self.max_charging+1, self.charge_no + 1)
        self.feature_no = 2
    ...
    def mask_state_value(self, next_state_values, state, reshape=False):
        if not reshape:
            return next_state_values - np.tile((state == 0) * 1e20, (self.power_no, 1, 1, 1)).flatten()
        else:
            next_state_values = torch.reshape(next_state_values, [-1, self.power_no, self.charge_no, self.battery_no, self.active_no])
            next_state_values = next_state_values - np.tile((state == 0) * 1e20, (self.power_no, 1, 1, 1))
    ...
    def random_action(self, action_idx):
        ...
        action = np.ravel_multi_index([power_idx] + action_idx, (self.power_no, self.charge_no, self.battery_no, self.active_no)) # charging_rate 고려 -> random choice 상태
...
class WPTSys:
    def __init__(self, env):
        ...
        self.time_power = np.zeros(env.dev_no)

        self.env = env
        self.time_power = self.env.ch_rate
    # 시스템 업데이트
    def update(self, scheduling, power_rate, training=True):
        ...
        # outage 계산
        self.outage[self.arrive_iter + self.env.iteration] = np.count_nonzero(tmp_batt <= 0)
        ...
        reward = - self.outage[self.arrive_iter + self.env.iteration] * 5 - np.sum(self.curr_batt <= self.batt_cautious_lvl)
        ...
        self.time_power = self.env.ch_rate * power_rate
    ...
    def get_state(self, next_state=False):
        # state 정의
        state = np.zeros((self.env.charge_no,self.env.battery_no, self.env.active_no))
        # 각 state 해당하는 유저 저장할 리스트 생성
        corr_user = [[[[] for _ in range(self.env.active_no)] for _ in range(self.env.battery_no)] for _ in range(self.env.charge_no)]

        # 각 유저별로 state 확인
        for dev_idx in range(self.env.dev_no):
            if next_state is True:
                p_idx, battery_idx, active_idx = self.get_state_idx(self.time_power[dev_idx],
                                                                    self.curr_batt[dev_idx],
            ...
            state[p_idx, battery_idx, active_idx] = 1
            # state 해당하는 유저에 현재 유저 인덱스 추가
            corr_user[p_idx][battery_idx][active_idx].append(dev_idx)

        return state, corr_user

    # 주어진 channel, mu에 따른 state idx 확인
    def get_state_idx(self, power, batt, active):
        battery_idx = np.max(np.nonzero(self.env.batt_partition <= batt))
        timepower_idx = np.max(np.nonzero(self.env.charge_partition <= power))
        if active:
            active_idx = 1
        else:
            active_idx = 0

        return timepower_idx, battery_idx, active_idx

    # descriptive action 변환
    def interpret_action(self, action, corr_user=None):
        action = list(np.unravel_index(action, (self.env.power_no, self.env.charge_no, self.env.battery_no, self.env.active_no)))
