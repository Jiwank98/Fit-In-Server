...
class Agent:
    ...
    def take_action(self, rf, cf, action_num,Q,q_avg,user):  # 엡실론 사용해서 random뽑기 / 나머지 확률로 높은거 뽑기
...
def load_users(epochs):
    ...
    with open("all_list_100000_19.pkl", "rb") as f:
        final_all_list = pickle.load(f)

    with open("users_rf_10000_19.pkl", "rb") as g:
...
def run(users, users_rf, users_cf, all_list, n_epi, recommend, agent,avg_q_list, reward_list,Q,q_avg):
    n = len(users)
    action1 = [0] * n
    action2 = [0] * n
    ...
    for user in range(n):
        rf = users_rf[user]
        cf = users_cf[n_epi]
        action1[user], action2[user],avg_q_value[user],q_avg = agent.take_action(rf, cf, agent.action_number,Q,q_avg,user)
    ...
    return users_rf, users_cf, all_list, recommend, avg_q_list, reward_list,Q,q_avg
...
path = 'ddqn_model_10000_20_save.pth'
agent= Agent(a)
...
q_avg= [[0]*5 for _ in range(2)]


for n_epi in range(epochs):
  users_rf,users_cf, all_list,recommend, avg_q_list, reward_list,Q,q_avg =  run(users,users_rf,users_cf, all_list,n_epi,recommend,agent,avg_q_list, reward_list,Q,q_avg)

for i in range(len(users)):
  print("user{} recommend:{}".format(users[i],recommend[i]))

with open("test_result_five6_w.pkl", "wb") as t:
