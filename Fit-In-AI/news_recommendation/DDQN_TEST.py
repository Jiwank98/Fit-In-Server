...
class Agent:
    ...
    def take_action(self, rf, cf, action_num,Q):  # 엡실론 사용해서 random뽑기 / 나머지 확률로 높은거 뽑기
        ...
        for i in range(0, action_num):
            inputs = np.concatenate([state, action[i].view(1)])
            inputs = torch.Tensor(inputs)
            out = Q.forward(inputs)
    ...
    def next_take_action(self, next_rf, next_cf, action_num,Q):
        ...
        q_list = []
        for i in range(0, action_num):
            inputs = np.concatenate([next_state, next_action[i].view(1)])
            inputs=torch.Tensor(inputs)
            out = Q.forward(inputs)
            q_list.append(out.item())
...
def load_users(epochs):
    ...
    with open("all_list_re.pkl", "rb") as f:
        final_all_list = pickle.load(f)

    with open("users_rf_re.pkl", "rb") as g:
...
def run(users, users_rf, users_cf, all_list, n_epi, answer, recommend, agent, buffer, avg_q_list, reward_list):
    ...
    n = len(users)
    action = [[0] * n for _ in range(2)]

    isnewsread = [[0] * 2 for _ in range(n)]
    avg_q_value = [0] * n
    rewards = [0] * n
    ...
    for user in range(n):
        rf = users_rf[user]
        cf = users_cf[n_epi]
        action[0][user], action[1][user], avg_q_value[user] = agent.take_action(rf, cf, agent.action_number,Q)
        avg_q_list[user].append(avg_q_value[user])

    # 유저별로 kakao톡에 추천 뉴스 보내주기
    for user in range(n):
        isnewsread[user] = kakaoFeedback(users_cf[n_epi], action[0][user], action[1][user], users[user])
    ...
    for user in range(n):
        ...
        for r in range(2):
            reward = 0
            a_user = [0] * 10
            if isnewsread[user][r] > 0:
                reward = 100
                a_user[action[r][user]] == 1
                all_user[action[r][user]] == 1
                if (n_epi >= 33):
                    answer[user][action[r][user]] += 1
                if users_cf[n_epi][1] == 1:
                    if action[r][user] == 9:
                        reward = 200

            recommend[user][action[r][user]] += 1
            all_reward += reward
            semi_all_list = np.zeros(40)
            semi_all_list = copy.deepcopy(all_list[user])
            semi_all_list.append(a_user)
            S_P = s_sp(n_epi, semi_all_list)
            next_action = agent.next_take_action(S_P, next_cf, agent.action_number,Q)
            state = np.concatenate((users_rf[user], users_cf[n_epi]))
            state_new = np.concatenate((S_P, next_cf))
            buffer.append((state, action[r][user], reward, state_new, next_action))

        all_list[user].append(all_user)
        rewards[user] += all_reward
        reward_list[user].append(rewards[user])
        next_state = s_sp(n_epi, all_list[user])
        users_rf[user] = next_state
        if (n_epi % 1000 == 0):
            print("epoch = {} / user {}'s RECOMMEND: {}, {}".format(n_epi, users[user], action[0][user],
                                                                        action[1][user]), end=" ")

    if (n_epi % 1000 == 0):
        print("---------------------------------------------------------------------------------------")

    if n_epi % Config.update_target_frequency == 0:
